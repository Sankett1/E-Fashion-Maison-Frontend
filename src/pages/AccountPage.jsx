...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ padding:"4px 12px", background:bg, border:`1px solid ${color}30`,
      fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"10px",
...
const Spinner = () => (
  <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 0", gap:12 }}>
    ...
    <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:12, letterSpacing:"0.16em", color:"#6b5c44" }}>
      LOADING…
    </span>
  </div>
);
        ...
        <div style={{ maxWidth:"1200px", margin:"0 auto", display:"flex", alignItems:"center", gap:"28px" }}>
          <div style={{ width:"80px", height:"80px", borderRadius:"50%", overflow:"hidden", flexShrink:0,
            ...
            {user.avatar?.url
              ...
              : <span style={{ fontFamily:"'Playfair Display',serif", fontSize:"28px", color:"#0f0c08", fontWeight:500 }}>
                  {user.name?.[0]?.toUpperCase() || "U"}
                </span>}
          </div>
          <div>
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"28px", fontWeight:400, color:"#fff", margin:"0 0 6px" }}>{user.name}</h1>
            <p style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"13px", color:"rgba(255,255,255,0.45)", letterSpacing:"0.08em" }}>
      ...
      <div className="r-section" style={{ maxWidth:"1200px", margin:"0 auto", paddingBottom:"80px" }}>
        ...
        {activeTab === "Profile" && (
          <div style={{ maxWidth:"600px" }}>
                ...
                <div key={key} style={{ marginBottom:"20px" }}>
                  ...
                  {editing && key !== "email"
                    ? <input value={editData[key] || ""} type={type} onChange={e => setEditData(d => ({ ...d, [key]: e.target.value }))}
                        style={{ width:"100%", padding:"12px 16px", boxSizing:"border-box", border:"1px solid rgba(201,168,76,0.35)", fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"15px", color:"#1a1208", outline:"none", background:"#fdfaf5" }}/>
                    : <p style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"15px", color:"#1a1208", padding:"12px 0", borderBottom:"1px solid rgba(201,168,76,0.1)", margin:0 }}>
                        {key === "email" ? user.email : (user[key] || "—")}
                      </p>}
                </div>
              ))}
              {profileMsg && (
                <div style={{ padding:"10px 14px", marginTop:8, background: profileMsg.type==="ok" ? "rgba(122,184,122,0.08)" : "rgba(220,100,100,0.08)", border:`1px solid ${profileMsg.type==="ok" ? "rgba(122,184,122,0.3)" : "rgba(220,100,100,0.3)"}`, fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"13px", color: profileMsg.type==="ok" ? "#7ab87a" : "#e07070" }}>
                  {profileMsg.text}
                </div>
              )}
            </div>
            ...
            <div style={{ background:"#fff", padding:"40px", border:"1px solid rgba(201,168,76,0.12)" }}>
              <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"22px", fontWeight:400, color:"#1a1208", marginBottom:"28px" }}>Change Password</h2>
              {[["Current Password","currentPassword"],["New Password","newPassword"],["Confirm New Password","confirm"]].map(([label,key]) => (
                <div key={key} style={{ marginBottom:16 }}>
                  <label style={{ display:"block", fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"9.5px", letterSpacing:"0.18em", color:"#6b5c44", marginBottom:8 }}>{label.toUpperCase()}</label>
                  <input type="password" value={pwForm[key]} onChange={e => setPwForm(f => ({ ...f, [key]: e.target.value }))}
                    style={{ width:"100%", padding:"12px 16px", boxSizing:"border-box", border:"1px solid rgba(201,168,76,0.25)", fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"14px", color:"#1a1208", outline:"none" }}/>
                </div>
              ))}
              {pwMsg && (
                <div style={{ padding:"10px 14px", marginBottom:12, background: pwMsg.type==="ok" ? "rgba(122,184,122,0.08)" : "rgba(220,100,100,0.08)", border:`1px solid ${pwMsg.type==="ok" ? "rgba(122,184,122,0.3)" : "rgba(220,100,100,0.3)"}`, fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"13px", color: pwMsg.type==="ok" ? "#7ab87a" : "#e07070" }}>
                  {pwMsg.text}
                </div>
              )}
              <button onClick={handleChangePassword} disabled={pwSaving}
                style={{ padding:"12px 28px", background:C.gold, border:"none", color:"#0f0c08", fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"10px", letterSpacing:"0.18em", cursor: pwSaving ? "not-allowed" : "pointer", opacity: pwSaving ? 0.7 : 1 }}>
        ...
        {activeTab === "My Orders" && (
          <div>
            ...
            {ordersError && <div style={{ padding:"16px", background:"rgba(220,100,100,0.06)", border:"1px solid rgba(220,100,100,0.25)", fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"14px", color:"#e07070" }}>{ordersError}</div>}
            {!ordersLoading && !ordersError && orders.length === 0 && (
              <div style={{ textAlign:"center", padding:"60px 0" }}>
                <div style={{ fontSize:48, marginBottom:16 }}>📦</div>
                <p style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#6b5c44", marginBottom:20 }}>No orders yet</p>
            ...
            {orders.map(order => (
              <div key={order._id} style={{ background:"#fff", padding:"24px 28px", border:"1px solid rgba(201,168,76,0.12)", marginBottom:"14px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"24px", flexWrap:"wrap" }}>
                <div>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"17px", color:C.gold, marginBottom:"4px" }}>
                    #{order.orderNumber || order._id.slice(-8).toUpperCase()}
                  </div>
                  <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"12px", color:"#b0a08a" }}>
                ...
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"18px", color:"#1a1208", marginBottom:"6px" }}>₹{Number(order.totalAmount).toLocaleString("en-IN")}</div>
        ...
        {activeTab === "Wishlist" && (
          <div>
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"24px", fontWeight:400, color:"#1a1208", marginBottom:"28px" }}>Saved Items ({wishlist.length})</h2>
            {wishlist.length === 0 ? (
              <div style={{ textAlign:"center", padding:"60px 0" }}>
                <div style={{ fontSize:48, marginBottom:16 }}>🤍</div>
                <p style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#6b5c44", marginBottom:20 }}>Your wishlist is empty</p>
                <button className="m-btn-gold" onClick={() => navigate("/shop")}>EXPLORE COLLECTION</button>
              </div>
            ) : (
                ...
                {wishlist.map(item => (
                  <div key={item._id} style={{ background:"#fff", border:"1px solid rgba(201,168,76,0.1)", overflow:"hidden" }}>
                    ...
                    <div style={{ padding:"16px" }}>
                      <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"9px", letterSpacing:"0.14em", color:"#6b5c44", marginBottom:4 }}>{item.category?.toUpperCase()}</div>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"16px", color:"#1a1208", marginBottom:"6px" }}>{item.name}</div>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"15px", color:"#3a2e1e", marginBottom:"14px" }}>₹{Number(item.price).toLocaleString("en-IN")}</div>
        ...
        {activeTab === "Addresses" && (
          <div style={{ maxWidth:"700px" }}>
            ...
            {addrMsg && <div style={{ padding:"10px 14px", marginBottom:16, background: addrMsg.type==="ok" ? "rgba(122,184,122,0.08)" : "rgba(220,100,100,0.08)", border:`1px solid ${addrMsg.type==="ok" ? "rgba(122,184,122,0.3)" : "rgba(220,100,100,0.3)"}`, fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"13px", color: addrMsg.type==="ok" ? "#7ab87a" : "#e07070" }}>{addrMsg.text}</div>}
            {addingAddr && (
              <div style={{ background:"#fff", padding:"28px", border:"1px solid rgba(201,168,76,0.15)", marginBottom:20 }}>
                <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:"18px", fontWeight:400, color:"#1a1208", marginBottom:20 }}>New Address</h3>
                {[["Label","label","e.g. Home"],["Address","address","Street, Area, Landmark"]].map(([label,key,ph]) => (
                  <div key={key} style={{ marginBottom:12 }}>
                    <label style={{ display:"block", fontFamily:"'Cormorant Garamond',serif", fontSize:"9px", letterSpacing:"0.16em", color:"#6b5c44", marginBottom:6 }}>{label.toUpperCase()}</label>
                    <input value={addrForm[key]} placeholder={ph} onChange={e => setAddrForm(f => ({ ...f, [key]: e.target.value }))}
                      style={{ width:"100%", padding:"10px 14px", boxSizing:"border-box", border:"1px solid rgba(201,168,76,0.25)", fontFamily:"'Cormorant Garamond',serif", fontSize:"14px", color:"#1a1208", outline:"none" }}/>
                  </div>
                ))}
                ...
                <button onClick={handleAddAddress} disabled={addrSaving}
                  style={{ padding:"12px 28px", background:C.gold, border:"none", color:"#0f0c08", fontFamily:"'Cormorant Garamond',serif", fontSize:"10px", letterSpacing:"0.18em", cursor: addrSaving ? "not-allowed" : "pointer", opacity: addrSaving ? 0.7 : 1 }}>
                  {addrSaving ? "SAVING…" : "SAVE ADDRESS"}
                </button>
              </div>
            )}
            {addresses.length === 0 && !addingAddr && <div style={{ textAlign:"center", padding:"40px 0", fontFamily:"'Cormorant Garamond',serif", fontSize:15, color:"#b0a08a" }}>No saved addresses yet.</div>}
            {addresses.map(addr => (
              <div key={addr._id} style={{ background:"#fff", padding:"24px 28px", border:"1px solid rgba(201,168,76,0.12)", marginBottom:"14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  <span style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"9px", letterSpacing:"0.2em", background:"rgba(201,168,76,0.1)", color:C.gold, padding:"3px 10px", marginBottom:"10px", display:"inline-block" }}>{(addr.label||"ADDRESS").toUpperCase()}</span>
                  <p style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"14px", color:"#3a2e1e", lineHeight:1.6, marginTop:"6px" }}>
                    {addr.address}{addr.city?`, ${addr.city}`:""}{addr.state?`, ${addr.state}`:""}{addr.pincode?` - ${addr.pincode}`:""}
                  </p>
                </div>
                <button onClick={() => handleRemoveAddress(addr._id)} style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"11px", color:"#e07070", background:"none", border:"none", cursor:"pointer", flexShrink:0 }}>Delete</button>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ maxWidth:"600px" }}>
            ...
            {[
              ...
              <div key={row.title} style={{ background:"#fff", padding:"24px 28px", border:"1px solid rgba(201,168,76,0.12)", marginBottom:12 }}>
                <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"16px", color:"#1a1208", marginBottom:4 }}>{row.title}</div>
                <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"13px", color:"#6b5c44" }}>{row.value}</div>
              </div>
            ))}
            <div style={{ marginTop:"28px", paddingTop:"24px", borderTop:"1px solid rgba(201,168,76,0.12)" }}>
              <button onClick={logout} style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"12px", color:"#e07070", background:"none", border:"1px solid rgba(220,100,100,0.3)", padding:"10px 22px", cursor:"pointer", letterSpacing:"0.1em", transition:"all 0.2s" }}
